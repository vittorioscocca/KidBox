  ...
  <section>
    ...
    <div class="grid2">
      <div class="card"><h3>Click e impressioni da Google <small>28 gg · Search Console pubblica con 2-3 gg di ritardo</small></h3>
        ...
        ${S?.goals ? `<div class="tablewrap"><table><tr><th>obiettivo al ${itDate(S.goals.by)}</th><th class="n">oggi</th><th class="n">traguardo</th><th></th></tr>
        ${S.goals.items.map((g) => { const done = g.value != null && (g.lowerIsBetter ? g.value <= g.target : g.value >= g.target); const v = g.value == null ? "—" : g.pct ? pct(g.value) : fmt(g.value, g.lowerIsBetter ? 1 : (g.value < 10 ? 1 : 0)); const t = g.pct ? pct(g.target) : fmt(g.target); const w = g.value == null || g.lowerIsBetter ? null : Math.min(100, Math.round((g.value / g.target) * 100)); return `<tr><td>${esc(g.label)}</td><td class="n">${v}</td><td class="n muted">${t}</td><td>${done ? `<span class="pill ok">raggiunto</span>` : w != null ? `<div class="fbar" style="height:8px;min-width:80px"><div class="ffill" style="width:${w}%"></div></div>` : ""}</td></tr>`; }).join("")}
        </table></div>` : ""}
      </div>
