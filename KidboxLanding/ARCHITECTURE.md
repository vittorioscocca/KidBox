...
- **Footer «mappa del sito»** di `index.html`/`index-en.html` e delle pagine statiche (guida, supporto, privacy, termini, eliminazione dati; classe `sf-static`) — generato da `scripts/build_footer.py` (tra `<!-- footer:start -->` e `<!-- footer:end -->`, CSS tra `/* footer:css:* */`) con le categorie del blog e gli strumenti; lo lanciano all'inizio `build_tools.py` e `build_blog.py`, che poi copiano il footer nelle loro pagine puntando il link della lingua (`data-lang-other`) alla pagina gemella. Non modificare il footer a mano.
- **`public/assets/chat.js`** — chat «Chiedi a KidBox» in basso a destra su home, guida, supporto, strumenti e blog (i generatori la aggiungono da soli). Le domande suggerite hanno risposta scritta nel file, nelle 4 lingue, e non costano token; il resto va a `/api/chat`, rewrite sulla function `landingChat` (`functions/landingChat/`: base di conoscenza in `knowledge.md`, listino letto vivo da `config/plans`, cache delle risposte, limiti per sessione/IP, tetto di 1 $ al giorno). Statistiche e domande più frequenti nel report `scripts/console-daily-report.js`. Servita `no-cache`. Nascosta finché è aperto il banner dei cookie. La privacy ne parla nella sezione `#chat`. **Quando cambia il prodotto, aggiorna `knowledge.md`**: il modello sa solo quello.
