...
- **Footer «mappa del sito»** di `index.html`/`index-en.html` — generato da `scripts/build_footer.py` (tra `<!-- footer:start -->` e `<!-- footer:end -->`, CSS tra `/* footer:css:* */`) con le categorie del blog e gli strumenti; lo lanciano all'inizio `build_tools.py` e `build_blog.py`, che poi copiano il footer nelle loro pagine puntando il link della lingua (`data-lang-other`) alla pagina gemella. Non modificare il footer a mano.
