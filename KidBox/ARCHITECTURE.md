...
|---|---|---|
| Documenti | Blob Storage AES-GCM | title, fileName, mimeType, fileSize, storagePath, downloadURL, **extractedText**, **notes**, visibilityScope, createdBy/updatedBy |
...
| Wallet → Documenti d'identità (§11.6) | Blob Storage AES-GCM (stesso di Documenti) **+** `notes` = `kb_wallet_doc:<kind>\|enc=<base64 AES-GCM>` — Codice Fiscale, nome, nascita, numero documento, date, categorie patente sono cifrati con la chiave famiglia (`WalletCryptoService`) | solo il **kind** (es. "patente") resta in chiaro nel prefisso di `notes`, per filtrare/mostrare l'icona senza decifrare |
...
### 11.6 Wallet: Documenti d'identità (Tessera Sanitaria, CIE, Carta d'identità, Patente, Passaporto, Codice Fiscale)

Sotto-sezione in `Features/Wallet/Documents/`, costruita sopra lo stack Documents (§8.5) **senza nuovo modello SwiftData né migrazione**: i documenti restano `KBDocument`, distinti dai file generici via un tag in `notes`.

1. **`KBWalletDocumentKind`** (`Data/Models/Document/KBWalletDocumentKind.swift`): enum `tesseraSanitaria|cartaIdentita|cie|passaporto|codiceFiscale|patente|altro`, icona/colore per la card (patente = rosa, come il documento reale).
2. **`KBWalletDocumentMetadata`**: kind + `codiceFiscale?` + `holderName?` + `birthInfo?` + `documentNumber?` + `issueDate?`/`expiryDate?` + `patenteCategories: [KBPatenteCategory]` (categoria/rilascio/scadenza per ogni patente posseduta) + `notifyBeforeExpiry: Bool`.
   **Cifratura**: `KBDocument.notes` contiene `kb_wallet_doc:<kind>|enc=<base64>` — solo il `kind` resta in chiaro (serve a filtrare/mostrare l'icona senza decifrare), il resto è **AES-GCM con la chiave della famiglia** (`WalletCryptoService.encryptString/decryptString`, la stessa che protegge il PDF). `KBDocument.walletMetadata` (get/set) cifra/decifra automaticamente usando `familyId` del documento + uid corrente (`Auth.auth().currentUser?.uid`). Retrocompatibile col vecchio formato in chiaro (`cf=...|holder=...` subito dopo il kind, senza `enc=`). Nessuna modifica a `RemoteDocumentDTO`/sync: `notes` è un campo opaco già sincronizzato — cifrandone il contenuto qui, i dati sono protetti sia in locale (SwiftData) sia su Firestore.
3. **Acquisizione** (`WalletDocumentScannerView` + `VNDocumentCameraViewController`): scanner di sistema con rilevamento bordi/prospettiva. UI guida fronte+retro per tutti i documenti, 2-3 pagine per il passaporto; pulsante "Aggiungi pagine" per accodare un'altra sessione di scansione.
4. **Estrazione automatica** (`WalletDocumentExtractor`): `VNDetectBarcodesRequest` (Code 39, CF sulla Tessera Sanitaria) + `VNRecognizeTextRequest` OCR; `NSDataDetector` per le date (più robusto di una regex fatta a mano) associate per vicinanza alle etichette "rilascio/emissione" o "scadenza/validità". Per la **patente**: legge dal fronte solo cognome/nome (campi 1/2), data e luogo di nascita (campo 3, → `birthInfo`) e numero patente (campo 5, non da 4a/4b/4c); dal **retro** legge la tabella con `parsePatenteTable` usando le **posizioni (bounding box)** delle righe OCR per associare categoria (colonna 9) → rilascio (colonna 10) → scadenza (colonna 11); se il retro non è leggibile, fallback alle categorie del campo 9 del fronte (senza date, da compilare a mano). La patente non ha Codice Fiscale (non estratto/non mostrato).
5. **Lettura assistita AI** (`WalletDocumentAIExtractor`, solo piano **Max**): invia le immagini scansionate alla Cloud Function `askAI` esistente (modello Haiku 4.5, nessun deploy dedicato) con un prompt strutturato che restituisce JSON; più precisa dell'OCR deterministico su tabelle/patente. Costo in "unità messaggio" del contatore famiglia = `n_immagini + 1` (stessa formula del server, mostrata all'utente prima di confermare); il contatore si aggiorna da solo via `AIService`/`AIUsageStore` (nessuna scrittura Firestore lato client).
6. **Salvataggio** (`AddWalletDocumentSheet`): replica il pattern upload di `DocumentFolderViewModel.uploadSingleFileFromData` (cifra il PDF con `DocumentCryptoService`, upload `DocumentStorageService`, crea `KBDocument` dentro la cartella auto-creata "Documenti d'identità" via `WalletIdentityFolder.findOrCreate`).
7. **Import di un documento già esistente** (`LinkExistingWalletDocumentSheet` + `WalletDocumentPickerSheet`, browser cartelle/documenti in sola lettura): tagga un `KBDocument` già caricato e lo **sposta** (stesso file, non duplicato) nella cartella "Documenti d'identità" — resta visibile sia in Documenti sia nel Wallet.
8. **UI**: `WalletHomeView` ha un segmented control Biglietti/Documenti; `WalletDocumentsSectionView` mostra le card sovrapposte verticalmente (stesso stack-con-peek di `WalletTicketCardView`/`cardStack`) con selezione multipla + hard delete (`DocumentDeleteService.deleteDocumentHard`, lo stesso di §8.5, pulsanti "Seleziona"/"+" affiancati come nella sezione Documenti) e context menu per l'eliminazione singola.
9. **Dettaglio** (`WalletDocumentDetailView`): barcode Code 39 rigenerato via `Code39Generator` (CoreImage non offre un generatore Code 39 nativo — disegno manuale delle barre in `WalletBarcodeView.swift`) con vista a tutto schermo dedicata (`WalletDocumentBarcodeFullscreenView`); mostrato per Tessera Sanitaria ma **non per la CIE** (niente barcode, il CF compare solo come testo in `infoCard`). Per **patente, carta d'identità, CIE, Codice Fiscale** un secondo pulsante "Visualizza documento (fronte/retro)" apre `WalletDocumentImagesFullscreenView` (viewer generico, temato sul colore del kind) con le **immagini vere scansionate** in un pager sfogliabile — non un facsimile ricostruito. Righe titolare/nascita/numero documento in `infoCard`; date di rilascio/scadenza (o tabella categorie per la patente) con toggle "avvisami una settimana prima"; pulsante "Modifica" apre `EditWalletDocumentSheet` con tutti i campi editabili; visualizzatore immagine/PDF diretto (decripta + `QuickLookPreview`, stessa logica di `DocumentFolderViewModel.open()` semplificata, condivisa in `WalletDocumentFileLoader`).
10. **Promemoria** (`WalletDocumentReminderService`): notifica locale singola a **T-7 giorni** dalla scadenza (per la patente, dalla scadenza più imminente tra le categorie), identificatori idempotenti `walletdoc.<documentId>.<offsetSec>`, rispetta sia il toggle per-documento sia la preference globale `kb_notifyOnWalletReminder` (stessa di `WalletReminderService` per i biglietti).
