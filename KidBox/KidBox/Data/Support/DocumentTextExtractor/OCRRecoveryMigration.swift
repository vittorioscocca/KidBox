...
enum OCRRecoveryMigration {
    ...
    private static let lifeAreaMigrationKey = "kb_life_area_ocr_recovery_v1_done"
    ...
    /// Allegati Casa / Garage / eventi animali senza testo estratto (parity Android backfill life-area).
    static func runLifeAreaIfNeeded(modelContext: ModelContext) async {
        if UserDefaults.standard.bool(forKey: lifeAreaMigrationKey) {
            return
        }
        ...
        let allDocuments = (try? modelContext.fetch(descriptor)) ?? []
        let candidates = allDocuments.filter { doc in
            guard isLifeAreaTaggedDocument(doc) else { return false }
            return doc.extractedText?.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty ?? true
        }

        KBLog.storage.kbInfo("OCR life-area recovery migration v1 start candidates=\(candidates.count)")
        if candidates.isEmpty {
            UserDefaults.standard.set(true, forKey: lifeAreaMigrationKey)
            KBLog.storage.kbInfo("OCR life-area recovery migration v1 completed (no candidates)")
            return
        }
        ...
        UserDefaults.standard.set(true, forKey: lifeAreaMigrationKey)
        KBLog.storage.kbInfo("OCR life-area recovery migration v1 completed")
    }
